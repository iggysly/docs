...
Если какая-нибудь проверка не проходит успешно, то сервис возвращает код ошибки (см. Табл. 11) в виде JSON следующего формата: 
...
Табл. 5. Параметры GET-запроса статуса отправленного сообщения (сегмента сообщения) 
...
Табл. 6. Параметры ответа на запрос статуса сообщения  
...
Табл. 7. Параметры GET-запроса на получение сообщений за период  
...
Табл. 8. Параметры GET-запроса на формирование статистики за период  
...
В Таблице 9 ниже приводится полный список параметров запроса.
Табл. 9. Параметры запроса на отправку Viber-сообщения
...
|                        Необязательные параметры                                                            |
+--------------------+------------+--------------------------------------------------------------------------+
| Validity           |  Int       | Время жизни сообщения (в секундах). Минимальное значение 30.             |
+--------------------+------------+--------------------------------------------------------------------------+
...
Если какая-нибудь проверка не проходит успешно, то Сервис возвращает Код ошибки (см.Табл. 13) в виде JSON следующего формата:
...
В Таблице 10 приводится полный список параметров запроса.

**Табл. 10. Параметры POST-запроса на отправку Viber-сообщения на несколько номеров**
...
Табл. 13. Коды возврата обработки сообщения в рамках запроса (Viber-сообщения)
